...
from tealql.security._inner_txn import (
    inner_txn_field_assigns,
    value_is_zero_address,
)
from tealql.security._program_shape import loc
...
class InnerTxnCloseRekeyViolation:
    ...
    def pretty(self) -> str:
        return (
            f"itxn_field {self.field}@{loc(self.assignment)}  "
...
class InnerTxnCloseRekeyDetector:
    ...
    def detect(self) -> list[InnerTxnCloseRekeyViolation]:
        ...
        for fs in inner_txn_field_assigns(self.prog, file=self.file):
            if fs.field not in _DANGEROUS_FIELDS:
                continue
            if value_is_zero_address(self.prog, fs.value, file=self.file):
