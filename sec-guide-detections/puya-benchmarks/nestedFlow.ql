...
predicate isTestSource(Dataflow::Node n) {
  // Only consider nodes in the nested test file (lines 1..47 approximately).
  n.getUnderlyingASTNode().getLocation().getStartLine() <= 50 and
  (
...
predicate isTestSink(Dataflow::Node n) {
  n.getUnderlyingASTNode() instanceof TOpcode_app_global_put and
  n.getUnderlyingASTNode().getLocation().getStartLine() <= 50
}
