...
class AstNode instanceof TAstNode{
    ...
    int getNumberOfConsumedArgs() { result = 0 }

    int getNumberOfOutputArgs() { result = 0 }

    /** Gets all possible stack heights before this opcode executes. */
    cached
    int getStackHeightAt() {
        StackDepth::nodeStackDepth(this, result)
    }
...
class Subroutine extends AstNode{
    ...
    ProtoOpcode getAffectingProto() {
        result = this.getSubroutineStart().getNextLine()
    }

    /** Stack height relative to subroutine entry (entry = 0). For non-proto subroutines. */
    int getSubroutineRelativeStackHeight(AstNode n){
        not exists(this.getAffectingProto()) and
        (
            // Base case: entry point starts at 0
            n = this and result = 0
            or
            // Recursive case
            exists(AstNode prev, int prevHeight |
                prevHeight = this.getSubroutineRelativeStackHeight(prev) and
                n = getNextNode_subroutineAux(prev) and
                (
                    // For callsub with proto
                    exists(ProtoOpcode proto |
                        proto = prev.(CallsubOpcode).getSubroutine().getAffectingProto() and
                        result = prevHeight - proto.getNumberOfSubroutineInputArgs() + proto.getNumberOfSubroutineOutputArgs()
                    )
                    or
                    // For callsub without proto: recurse into the called sub
                    exists(Subroutine calledSub, int retsubHeight |
                        calledSub = prev.(CallsubOpcode).getSubroutine() and
                        not exists(calledSub.getAffectingProto()) and
                        retsubHeight = calledSub.getSubroutineRelativeStackHeight(calledSub.getRetsubOpcode()) and
                        result = prevHeight + retsubHeight
                    )
                    or
                    // All other opcodes
                    not prev instanceof CallsubOpcode and
                    result = prevHeight + prev.getStackDelta()
                ) and
                result >= -1000 and result <= 1000
            )
        )
    }

    int getFrameRelativeSubroutineStackHeight(AstNode n){
        // Base case: at subroutine entry, height = proto input args
        n = this and result = this.getAffectingProto().getNumberOfSubroutineInputArgs()
        or
        // Recursive case: propagate through subroutine-local CFG
        exists(AstNode prev, int prevHeight |
            prevHeight = this.getFrameRelativeSubroutineStackHeight(prev) and
            n = getNextNode_subroutineAux(prev) and
            (
                // For callsub: use the called subroutine's proto if available
                exists(ProtoOpcode proto |
                    proto = prev.(CallsubOpcode).getSubroutine().getAffectingProto() and
                    result = prevHeight - proto.getNumberOfSubroutineInputArgs() + proto.getNumberOfSubroutineOutputArgs()
                )
                ...
                or
                // For callsub without proto: use the called sub's relative stack height to compute net effect
                exists(Subroutine calledSub, int retsubHeight |
                    calledSub = prev.(CallsubOpcode).getSubroutine() and
                    not exists(calledSub.getAffectingProto()) and
                    retsubHeight = calledSub.getSubroutineRelativeStackHeight(calledSub.getRetsubOpcode()) and
                ...
                or
                // For all other opcodes, use their stack delta
                not prev instanceof CallsubOpcode and
                result = prevHeight + prev.getStackDelta()
            ) and
            result >= -1000 and result <= 1000
