...
Companion to :mod:`tealtools.dataflow.box`. Where box-out flow treats
...
Same substrate and caveats as :mod:`tealtools.dataflow.box`: taint-only
(not predicate-aware on its own -- compose with
:mod:`tealtools.dataflow.predicate_aware` to drop guarded flows), stops
...
# arities live in tealtools.opcode_sigs):
...
def detect_out_of_state_flows(
    ...
    *,
    ...
    :mod:`tealtools.dataflow.predicate_aware` to suppress flows a
