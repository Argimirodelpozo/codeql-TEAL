...
# ---------------------------------------------------------------------------
# Program-type inference (Application vs. LogicSig)
# ---------------------------------------------------------------------------


# Opcodes available *only* to stateful Applications. A program containing
# any of these is unambiguously an App; the absence of all of them is
# strong (though not formally proof) evidence that the program is a
# stateless LogicSig — pure-validation contracts never need to read or
# mutate Algorand state, issue inner transactions, or query box / app /
# account params. Used by :func:`infer_program_type` to pick which
# sec-guide detectors apply.
_APP_ONLY_OPS = frozenset({
    ...
    "app_opted_in",
    # Inner transactions (only Apps can issue these).
    ...
    "box_len", "box_get", "box_put", "box_splice", "box_resize",
    # App / account / asset params lookups.
    "app_params_get", "acct_params_get",
    "asset_params_get", "asset_holding_get",
    # min_balance (LogicSig can technically use it but it's an App idiom;
    # we'd rather over-classify as App and run more detectors than under-).
    "min_balance",
})
...
def infer_program_type(
    ...
    """Return ``"app"`` if the program (or the file-scoped slice of it)
    contains any opcode that's only available to stateful Applications,
    or any of the txn-field reads that are idiomatically App-only
    (``OnCompletion`` / ``ApplicationID`` / ``ApplicationArgs``). Else
    ``"logicsig"``.

    Two-tier discriminator:

    1. **App-only opcodes** — ``app_global_get`` / ``itxn_*`` / ``box_*``
       / etc. These are *physically* unavailable to LogicSigs.
    2. **App-idiomatic txn fields** — ``OnCompletion`` /
       ``ApplicationID`` / ``ApplicationArgs[N]``. LogicSigs can
       technically read these via ``txn`` / ``txna``, but in practice
       a LogicSig that does so is validating an app call on behalf of
       some App — App-targeted detectors (is_deletable, OnCompletion
       guards, ...) still apply to that LogicSig.

    Conservative: when in doubt, return "app" so no detector is
    silently skipped on a misclassified program."""
    for a in prog.assignments:
        if file is not None and a.location.file != file:
            continue
        if a.op in _APP_ONLY_OPS:
            return "app"
        if a.op in ("txn", "txna") and a.immediates:
            field = a.immediates.split()[0]
            if field in ("OnCompletion", "ApplicationID", "ApplicationArgs"):
                return "app"
    return "logicsig"
