...
:mod:`tealtools.xcontract` (top of the stack). They live in this folder for
historical reasons, but they are re-exported LAZILY (PEP 562 ``__getattr__``)
so that ``import tealtools.cfg`` / ``from tealtools.cfg import CFG`` does NOT
drag the whole cross-contract analysis layer (xcontract → auth_domination,
inner_txn_report) into the substrate. ``from tealtools.cfg import SuperCFG``
