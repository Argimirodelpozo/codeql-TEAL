...
(:func:`tealtools.dataflow.byte_taint.byte_taint`, ``validate=True``): taint is
...
from security.detections.tainted_fund_flow import TaintedFundFlowDetector
from tealtools.dataflow.byte_taint import Intervals, byte_taint
...
class PartialTaintedFundFlowDetector:
    ...
    def __init__(self, prog: SSAProgram, *, file: Optional[str] = None,
        ...
        if getattr(prog, "_materialized", False):
            raise ValueError(
                "PartialTaintedFundFlowDetector requires the pre-materialized SSA."
            )
        if getattr(prog, "_dead_eliminated", False):
            raise ValueError(
                "PartialTaintedFundFlowDetector requires the pre-dead-elimination SSA."
            )
        self.prog = prog
        self.file = file
