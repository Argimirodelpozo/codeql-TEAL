 ...
 * @description Detects TEAL contracts that allow DeleteApplication.
 *              A contract is deletable if OnCompletion == 5 (DeleteApplication)
 ...
 * DeleteApplication (OnCompletion == 5).
 *
 * CFG/dominance logic: We find approval exits (blocks ending in return that
 * may approve). For each, we check if an OnCompletion == 5 guard controls it.
 * The guard must exist and the "non-equality" branch (reject path) must
 * dominate the path to approval — so we only approve when OnCompletion != 5.
 */
