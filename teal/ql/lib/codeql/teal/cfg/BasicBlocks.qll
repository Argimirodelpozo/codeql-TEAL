...
private import codeql.teal.ast.AST as Ast
...
class BasicBlock extends TBasicBlockStart {
  ...
  predicate reaches(BasicBlock target){
    this.getASuccessor*() = target
  }
  ...
  predicate hasLocationInfo(string path, int sl, int sc, int el, int ec) {
    exists(Location loc | loc = this.getLocation() |
        path = loc.getFile().getAbsolutePath() and
        sl = loc.getStartLine() and
        sc = loc.getStartColumn() and
        el = this.getLastNode().getLocation().getEndLine() and
        ec = this.getLastNode().getLocation().getEndColumn()
    )
  }
}
...
private module JoinBlockPredecessors {
  private predicate id(Teal::AstNode x, Teal::AstNode y) { x = y }

  private predicate idOf(Teal::AstNode x, int y) = equivalenceRelation(id/2)(x, y)
