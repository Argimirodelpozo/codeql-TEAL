...
Tags SSAVars / Phis with an :class:`IntRange` + uint64 type from three seed
tables — ``_OP_RANGE_SEEDS`` (op alone bounds the value, e.g. bool-shaped
comparisons / ``getbyte`` / ``len``), ``_TXN_FIELD_RANGES`` (enum-valued
txn/gtxn/itxn fields) and ``_GLOBAL_FIELD_RANGES`` (``global FIELD``) — then
unions arg ranges through phis to a fixed point.
...
from ..ssa.models import _GLOBAL_FIELD_RANGES, _TXN_FIELD_RANGES
...
def propagate_ranges(prog: SSAProgram) -> None:
    ...
    # Pass 1: seed from per-op rules. Single-output guard reflects that every
    # range-yielding op here produces exactly one stack output.
