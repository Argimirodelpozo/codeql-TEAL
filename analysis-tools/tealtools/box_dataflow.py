...
Reuses the :class:`Source` / :class:`Sink` / :class:`FlowRule` taint
framework from :mod:`teal_nonunique_box_key`. The detector class
itself (``NonUniqueBoxKeyDetector``) is fully general — its name
just reflects the original use case. Here we feed it different
source/sink sets to ask different questions.
...
from teal_nonunique_box_key import (
...
from teal_ssa import Assignment, Const, Phi, SSAProgram
...
#
# Despite the field name ``key_input_index``, this is the generic
...
def detect_into_box_flows(
    ...
    *,
    ...
    return NonUniqueBoxKeyDetector(
        prog,
...
def detect_correlated_flows(
    ...
    *,
    ...
    if not synth_sources:
        return []
    pass2 = NonUniqueBoxKeyDetector(prog, sources=synth_sources, sinks=sinks)
    ...
    for v in flat_violations:
        ...
        out.append(CorrelatedViolation(
            ...
            sink_operand=v.sink_key_operand,
        ))
    return out
