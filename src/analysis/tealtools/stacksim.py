...
Sits on top of :mod:`tealtools.ssa`. For every :class:`tealtools.ssa.Assignment`,
...
identity convention :mod:`tealtools.ast` uses, so a :class:`StackSnapshot`
can sit beside an :class:`tealtools.ast.AstNode` in any analysis-pass index.
...
class StackSnapshot:
    ...
    Identity is ``(file, line)``, matching :class:`tealtools.ast.AstNode` and
    :class:`tealtools.ssa.Assignment`. ``assignment`` back-references the SSA
    assignment so consumers can pivot to the typed inputs/outputs view.
    ``ast_node`` is the matching :class:`tealtools.ast.AstNode` from the
    underlying :mod:`tealtools.graphs` graph (``None`` only if the line has
    ...
    ast_node: Optional[object] = None  # tealtools.ast.AstNode; not typed to avoid the import.
...
class StackSimulation:
    ...
    def _get_ast_index(self) -> dict[tuple[str, int], object]:
        ...
        # Late import: keeps tealtools.stacksim importable even if tealtools.ast
