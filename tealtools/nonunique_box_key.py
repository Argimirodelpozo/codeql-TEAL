...
Forward taint propagation on top of :class:`tealtools.ssa.SSAProgram`:
    ...
    2. **Propagation.** A taint flows through identity-preserving ops
       only — stack shuffles via :func:`tealtools.ssa._shuffle_mapping`,
...
class Source:
    ...
    after the op runs; matches :class:`tealtools.ssa.SSAVar`'s
...
class Sink:
    ...
    :class:`tealtools.ssa.Assignment`'s top-first ``inputs``).
    """
