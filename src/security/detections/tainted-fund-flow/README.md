...
Guard dominance is interprocedural for free (`PathPredicateAnalysis` propagates caller predicates across `callsub` edges), and so is the taint: the base SSA def-use relation leaves `frame_dig` disconnected, but `_user_input_taint` unions each `frame_dig` param read's taint from the caller args bound to it (`tealtools.passes.frame_flow.frame_param_sources`), so a value fed into a subroutine parameter and paid out inside the callee is caught natively — no IR lift, no per-detector supplement.
